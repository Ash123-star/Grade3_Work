...
void main() {
  for (final config in [(const Size(360, 640), 1.0), (const Size(412, 915), 1.5), (const Size(800, 1000), 1.0)]) {
    testWidgets('all routes fit ${config.$1} at scale ${config.$2}', (tester) async {
      ...
      final container = ProviderContainer(overrides: [
        sessionProvider.overrideWith((ref) => const Session(role: UserRole.admin)),
      ...
      final routes = [
        '/dashboard', '/profile', '/ai', '/logs', '/calendar', '/settings', '/admin',
        '/privacy', '/licenses', '/history', '/conflict', '/report', '/ai-usage', '/export',
        '/pending', '/disabled', '/expired',
        for (final resource in resourceTitles.keys) '/list/${Uri.encodeComponent(resource)}',
        for (final resource in resourceTitles.keys) '/item/${Uri.encodeComponent(resource)}/1',
